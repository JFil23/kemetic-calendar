...
void main() {
  ...
  test('migration copies global tombstones once and leaves legacy intact', () async {
    SharedPreferences.setMockInitialValues(<String, Object>{
    ...
    expect(
      prefs.getBool(CalendarUserScopedPrefs.userScopedMigrationDoneKey(userA)),
      isTrue,
    );
  ...
  test('read prefers user-scoped list and falls back to global', () async {
    ...
    expect(
      userBRead,
      <String>['global-cid'],
      reason: 'second user with no scoped key falls back to global until they write',
    );
  });
