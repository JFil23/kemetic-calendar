...
def main() -> None:
    ...
    report = {
        ...
        "explained_context_differences": [
            ...
            },
            {
                ...
                "difference": "The HTML capture uses Djed's older 0.71 layered host, forced 30 px practice peek, and decorative inner handle. Production now uses Follow the Sky's 0.58 standard host, the shared frame's computed foreground start, and only the outer host handle. Only the visible crop changes: the Djed stage remains 230 px, or 205 px when the full viewport is at most 720 px, with the 390 x 214 SVG uniformly scaled.",
                "reason": "The user's September 13 corrections explicitly supersede the older Djed sheet composition while retaining the authored graphic size, artwork, palette, copy, actions, footer, and single shared content scroll.",
            },
            {
    ...
    lines = [
        ...
        "- The September 13 Djed corrections supersede the HTML's older `.71` layered host, forced 30 px practice peek, and decorative inner handle. Production uses Follow the Sky's `.58` standard host, the shared frame's computed foreground start, and one outer handle. The viewport clips the authored 230 px stage (205 px only when the full viewport is at most 720 px); it never rescales the graphic to the remaining sheet height. Djed's artwork, styling, content, actions, footer, and single shared scroll remain intact.",
