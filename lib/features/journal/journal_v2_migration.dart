...
class JournalV2Migration {
  ...
  Future<MigrationResult> migrateAllEntries() async {
    ...
    try {
      ...
      for (final entry in entries) {
        ...
        // Convert to document
        final document = JournalDocument.fromPlainText(
          gregDate: entryMap['local_date'] as String,
          text: body,
          meta: entryMap['meta'] as Map<String, dynamic>? ?? {},
          updatedAt: DateTime.parse(entryMap['updated_at'] as String),
        );
  ...
  Future<bool> migrateEntryByDate(DateTime localDate) async {
    try {
      ...
      // Convert to document
      final document = JournalDocument.fromPlainText(
        gregDate: '${localDate.year}-${localDate.month.toString().padLeft(2, '0')}-${localDate.day.toString().padLeft(2, '0')}',
        text: body,
        meta: entry.meta ?? {},
